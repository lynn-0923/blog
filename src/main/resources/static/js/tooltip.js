(function (window) {
    function tooltip(ele, transitionObj, enterCallback, outCallback) {
        ...
        if (transitionObj && ({}).constructor.name === "Object") {
            var transition = transitionObj.transition || false, time = transitionObj.time || 200, timer = null
        }
        var els = document.querySelectorAll(ele), tipContent = document.createElement("div");
        Array.prototype.slice.call(els).forEach(function (el) {
            el.addEventListener("mouseenter", function () {
                var currenLeft = el.offsetLeft, currenTop = el.offsetTop, currenWidth = el.offsetWidth,
                    currenHeight = el.offsetHeight, context = el.getAttribute("data-tip"),
